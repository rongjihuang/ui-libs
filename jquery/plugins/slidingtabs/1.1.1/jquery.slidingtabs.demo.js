/*
 * Sliding Tabs 1.1.1 jQuery Plugin - http://codecanyon.net/item/sliding-tabs-jquery-plugin/141774
 * 
...
(function($) { 
	...
	$.fn.slideTabs = function(client_config) {	
        var default_config = {
            tabsList: 'ul.st_tabs',					
			viewContainer: 'div.st_view_container',		
			btnNext: 'a.st_next',
			btnPrev: 'a.st_prev',
			tabClass: 'st_tab',
			tabActiveClass: 'st_tab_active',
			viewActiveClass: 'st_active_view',
			btnDisabledClass: 'st_btn_disabled',
	...
	function SlideTabs($container, conf) {							
		...
		customize = {																					
			addTab: function() {
				...
				if (length == 2) { $removeBtn.removeClass().addClass('btn_enabled'); }
			...
			appendContent: function(id, text, span) {
				$tabs.append('<li><a href="#'+id+length+'" id="tab_'+length+'" class="st_tab">'+text+length+span+'</a></li>');																									
			...
			removeTab: function() {
				...
				// Enable/disable links					
				if (length == 30) { $addBtn.removeClass().addClass('btn_enabled'); }
