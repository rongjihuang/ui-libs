...
(function($) { 
	...
	function SlideTabs($container, conf) {							
		var $tabs = $container.find('.'+conf.classTabsList),
			$a = $tabs.children('li').find('a'),
			$content = $container.find('.'+conf.classViewContainer).css('overflow', 'hidden'), // Hide the overflowing content
		...
		var tabs = {												
			...
			initButtons: function() {				
				if (conf.buttonsFunction == 'slide') {
					...
					if (($lastElem.position()[val.obj] + $lastElem[val.func](true)) == (conf.slideLength - conf.offsetBR)) { this.disableButton($next); } 
